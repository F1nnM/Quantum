...
public class SecureSocket extends Socket {
    ...
    private int firstConnect() throws IOException, InterruptedException, GeneralSecurityException {
        ...
        if (deviceID != null) //TODO add option for wrong username and password combo
            send = Bytes.concat(username.getBytes(), Util.delimiterA, password.getBytes(), Util.delimiterA, deviceID);
        else
            send = Bytes.concat(username.getBytes(), Util.delimiterA, password.getBytes());
        ...
        if (deviceID == null) { //TODO Store session and device ID
    ...
    public void disconnect() throws IOException, GeneralSecurityException {//TODO close automatically
        timer.cancel();
        byte[] tmp = enc.encryptByte(new byte[]{Util.endConnection});
        out.writeInt(tmp.length);
        out.write(tmp);
        out.flush();
        out.close();
